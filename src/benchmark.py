...
def benchmark_attention_ablation(save_csv=True, preferred_device=None):
    device = select_device(preferred_device)
    print(f"Using device: {device}\n")
    results = []

    att_modes = ['ca', 'sa', 'ul', 'stage']

    start_all = time.time()
    for mode in att_modes:
        args.att_mode = mode

        # 实例化对应 attention 子模块，使用 args.n_feats 作为 channel
        try:
            if mode in ['ca', 'sa', 'ul']:
                att_module = ULAttention(channel=args.n_feats, att_mode=mode)
            elif mode == 'stage':
                att_module = StageULAttention(channel=args.n_feats, att_mode='stage')
            else:
                raise ValueError(f"Unsupported attention mode: {mode}")
            att_module = att_module.to(device)
            att_module.eval()
            att_params = sum(p.numel() for p in att_module.parameters() if p.requires_grad)
        except Exception as e:
            warnings.warn(f"Failed to instantiate attention module for mode '{mode}': {e}")
            att_params = 0
            att_module = None

        # 计算 FLOPs 和推理时间（仅针对 attention 模块）
        try:
            if att_module is not None:
                # 使用 att_module 输入通道数作为 dummy tensor 通道数
                dummy_input = torch.randn(1, args.n_feats, 96, 96).to(device)

                # FLOPs 统计
                try:
                    flops, _ = profile(att_module, inputs=(dummy_input,), verbose=False)
                except Exception as e:
                    warnings.warn(f"FLOPs profiling failed: {e}")
                    flops = 0.0
            ...
            warnings.warn(f"Benchmark failed for attention mode '{mode}' with error: {e}")
            flops = 0.0
            fps = 0.0
            median_time = 0.0

        results.append({
            "Model": f"EDSR_VARIANTS_{mode}",
            "Use_DWConv": False,
            "Use_Attention": True,
            "Params (K)": att_params / 1e3,
            "FLOPs (G)": flops / 1e9 if flops else 0.0,
            "FPS": fps,
            "Inference Time (ms)": median_time,
        })
    ...
    os.makedirs(results_dir, exist_ok=True)
    now_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    csv_path = f"{results_dir}/benchmark_attention_ablation_{now_str}.csv"
    latest_path = f"{results_dir}/benchmark_attention_ablation_latest.csv"
    ...
    print(f"\n✅ Attention ablation results saved to {csv_path}")
    print(f"   Total time: {total_duration:.2f}s\n")
